...
/*  open and read monthly  analyzed fields */

if (  ! (monthcode[0] == '0' && monthcode[1] == 0))  {
...
/* Construct a cdf header */
  ...
  strncpy(cdf.title,"World Ocean Atlas 2005", 10);
...
/*  loop for each output cdf file */
   ...
   for (jj = 0; jj < 18; ++jj) {
	...
	if (*outdir != '\0') 
	   sprintf(filename, "%s/%4d.woa05_%2s.cdf", outdir, msq10, monthcode);
	else
	   sprintf(filename, "%4d.woa05_%2s.cdf", msq10, monthcode);
...
{
   fprintf(stderr,"\nConverts World Ocean Atlas 2005 1-deg analyzed fields");
   fprintf(stderr,"\nto HydroBase *.cdf files.  Output files are named by");
   fprintf(stderr,"\nto 4-digit WMOsq with extent .woa05.cdf");
