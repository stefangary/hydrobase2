...
/* Construct a cdf header */
  ...
  strncpy(cdf.title,"World Ocean Atlas 2001", 10);
...
/*  loop for each output cdf file */
   ...
   for (jj = 0; jj < 18; ++jj) {
	...
	if (*outdir != '\0') 
	   sprintf(filename, "%s/%4d.woa01.cdf", outdir, msq10);
	else
	   sprintf(filename, "%4d.woa01.cdf", msq10);
...
{
   fprintf(stderr,"\nConverts World Ocean Atlas 2001 1-deg analyzed fields");
   fprintf(stderr,"\nto HydroBase *.cdf files.  Output files are named by");
   fprintf(stderr,"\nto 4-digit WMOsq with extent .woa01.cdf");
