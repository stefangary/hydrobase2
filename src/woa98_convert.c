...
.   
.   USAGE: woa98_convert -O<outdir> -D<indir> 
...............................................................................
*/
#include <stdio.h>
#include <stdlib.h>
 ...
 /*   nitrate */
   ...
   for (zlev = 0; zlev < NSD; ++zlev) {
      for (row = 0; row < 180; ++row) {
	    ...
	    if (fscanf(infile, "%f", xptr) != 1){
	      fprintf(stderr,"\nError reading ni00an1 at row[%d],col[%d],zlev[%d]\n", row, col, zlev);
	      exit(1);
	    }
	    if (*xptr < -99.0)
	       *xptr =  HB_f_mask;
 ...
 /*   silicate */
   ...
   for (zlev = 0; zlev < NSD; ++zlev) {
      for (row = 0; row < 180; ++row) {
	    ...
	    if (fscanf(infile, "%f", xptr) != 1){
	      fprintf(stderr,"\nError reading si00an1 at row[%d],col[%d],zlev[%d]\n", row, col, zlev);
	      exit(1);
	    }
	    if (*xptr < -99.0)
	       *xptr =  HB_f_mask;
 ...
 /*   phosphate */
   ...
   for (zlev = 0; zlev < NSD; ++zlev) {
      for (row = 0; row < 180; ++row) {
	    ...
	    if (fscanf(infile, "%f", xptr) != 1){
	      fprintf(stderr,"\nError reading p00an1 at row[%d],col[%d],zlev[%d]\n", row, col, zlev);
	      exit(1);
	    }
	    if (*xptr < -99.0)
	       *xptr = HB_f_mask;
...
/* Construct a cdf header */
  ...
  strncpy(cdf.title,"World Ocean Atlas 98", 10);
...
/*  loop for each output cdf file */
   ...
   for (jj = 0; jj < 18; ++jj) {
	...
	if (*outdir != '\0') 
	   sprintf(filename, "%s/%4d.woa98.cdf", outdir, msq10);
	else
	   sprintf(filename, "%4d.woa98.cdf", msq10);
...
{
   fprintf(stderr,"\nConverts World Ocean Atlas 1998 1-deg analyzed fields");
   fprintf(stderr,"\nto HydroBase *.cdf files.  Output files are named by");
   fprintf(stderr,"\nto 4-digit WMOsq with extent .woa_1deg.cdf");
