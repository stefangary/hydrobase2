...
. ...............................................................   */
{
     ...
     * since 1 db = 10,000 Pa */

    /* Get center pressure for interval by
     * computing slope of temp vs. pressure.*/
    tgrd = hb_grady(t,p,n,pav_ptr,&tav);

    /* Loop over each depth level in the profile...*/
    for (k=0; k < n; k++) {
      /* Use the mean pressure on the interval
       * found with hb_grady to compute the
       * reference pressure for theta.*/
      theta = hb_theta(s[k],t[k],p[k],*pav_ptr);

      /* Compute the specific volume anomaly
       * and density anomaly at this depth 
       * using the avg. pressure as ref. pres.
       * Multiply by 1.0e-8 at the end to convert
       * from units of x10-8 m^3/kg to simply m^3/kg.*/
      data[k] = hb_svan(s[k],theta,*pav_ptr,&sig) * 1.0e-8;
    }

    /* Get v(35,0,pav).  sig is the density
     * anomaly left over from the deepest (last)
     * iteration of the preceeding loop.  So, we
     * convert sig kg/m^3 to m^3/kg and then
     * find the difference with the second deepest
     * value of the specific volume anomaly profile.*/
    v350p = (1./(sig + 1000.))- data[n-1];

    /* Compute potential density anomaly gradient
     * by finding the slope between specific volume
     * anomaly and pressure. Since data = svan,
     * and d(svan)/dp = d(1/rho)/dp
     *                = -1*(rho)^-2*d(rho)/dp */
    dvdp = hb_grady(data,p,n,pav_ptr,&vbar);

    /* The average value of the specific volume
     * anomaly, vbar computed above, is then
     * added to the bottom difference computed
     * for v350p.*/
    vbar += v350p;

    /* This is a complicated conversion using
     * derivative rules in addition to unit
     * conversions:
     *
     * d(1/rho)/dP * dP/dz = d(1/rho)/dz = -(rho^-2) * d(rho)/dz
     *
     * so, rearranging, results in:
     *
     * d(1/rho)/dP * (-rho * g) * (-1 * rho^2) = d(rho)/dz
     *
     * but, the dP is currently in units of decibar
     * (standard for hydrographic observations)
     * so we need to convert the dP from db to Pa,
     *
     * 1 db = 10,000 Pa
     *
     * so the result is:
     *
     * d(1/rho)/dP * (1 db/10000 Pa) (rho * g) * (rho^2) = d(rho)/dz
     *
     * and we must finally multiply by g/rho to get:
     *
     * d(1/rho)/dP * (db2Pa) * (-rho*g) * (-1*rho^2) * (g/rho) = (g/rho)*d(rho)/dz
     *
     * which simplifies to:
     *
     * dvdp * -gr2 * rho^2 = (g/rho)*d(rho)/dz
     *
     * or
     *
     * dvdp * -gr2 / (1/rho)^2 = N^2
     */

    *e_ptr = -gr2 * dvdp/ (vbar * vbar);

    /* Check the sign and force positive
     * values. */
    mult = *e_ptr < 0 ? -1.0 : 1.0;
    
    /* Convert from rad per sec to cycles per hour
     * and apply sign convention.*/
...
.               for lat = 30 deg, depth = 7321.45 meters */
{
    ...
    /* Compute an intermediate constant, varies
     * with lat and is same as c1 in Saunders (1981). */
