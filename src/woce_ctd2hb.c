...
/*  parse the command line arguments */
   ...
   if (! (oflag && nfiles && sflag )) {
       fprintf(stderr,"\nYou must specify input file(s), sumfile, and output file.\n");
       exit(1);
   }
   ...
   if (t68flag)
      fprintf(stderr,"\nInput temperatures are not T90 ");
   else 
...
/* line 2 */
   ...
   if (xx == NULL) {
	...
	if (t68flag)
	  ...
	  fprintf(stderr,"\n...will assume it is T90 and apply conversion factor");
      }
      else
        t68flag = 1;
   }
     ...
     if (t68flag) {
       fprintf(stderr, "\nHeader says input temperature is T90 -- will apply conversion factor for HydroBase output.");
       t68flag = 0;
     }
   }  
   ...
   while ( fscanf(fptr, "%[^\n]", line) != EOF) {
	...
	++xx;
	 ...
	 case '8':    
      ...
      if (!t68flag)  /* convert from T90 to T68 */
         t[n] *= 1.00024;
