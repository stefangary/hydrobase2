  ...
  for (i = 1; i < argc; i++) {
    if (argv[i][0] == '-') {
	  ...
	  tension = atof(&argv[i][2]);
        ...
        case 'U':
          uflag = 1;
          error = (sscanf(&argv[i][2],"%lf", &topo_xinc) == 1) ? 0 : 1;
          topo_yinc = topo_xinc;
          st = &argv[i][2];
          while (*(st++) != '\0') {
            if (*st == '/') {
               ++st;
               error = (sscanf(st,"%lf", &topo_yinc) == 1) ? 0 : 1;
               break;
            }
          }
   ...
   error = 0;
    ...
    if (topofile == NULL ) {
       ...
       if (topofile == NULL) {
          ...
          fprintf(stderr,"You can specify the name of a binary topography file for input with [-W]\n"); 
          exit(1);
      }
...
/* Get array of standard depths from first cdf file in list */
   ...
   while (infile < 0 && i <= nfiles) {
     fprintf(stderr,"\nOpening netCDF file %s...",argv[i]);
     infile = cdf_open(dir, argv[i], extent, FALSE);
     ++i;
   }
   if (infile < 0) {
      fprintf(stderr,"ERROR: Unable to open any cdf files. Exiting...\n");
      exit(1);
    }
   ...
   if (verbose) {
     fprintf(stderr,"\nReading netCDF header info...");
   }
   if (error = read_cdf_hdr(infile, cdfin)) {
     fprintf(stderr,"\nERROR: in read_cdf_hdr. Exiting...\n");
         exit (1);
   }
   if (verbose) {
     fprintf(stderr,"\nFound %d levels in cdfin->nz.",cdfin->nz);
   }
   xtmp = (float *) calloc((size_t)cdfin->nz, sizeof(float));
   NSTDLEVS = read_cdf_depths(infile, xtmp);
   if (verbose) {
     fprintf(stderr,"\nRead %d depth levels, listing below:",NSTDLEVS);
   }
   for (i = 0; i < NSTDLEVS; ++i) {
     if (verbose) {
       fprintf(stderr,"\nLevel: %d at depth: %f",i,xtmp[i]);
     }
   ...
   for (i = 1; i < nprops; ++i) {
	...
	dimension and order. */  
	 ...
	 /* set pointers to define small work grids */
	     ...
	     *
             * Find corresponding isopycnal value from
	     * distance-weighted avg of surrounding nodes.
	     *
	     * Then interpolate to find that isopycnal at
	     * all the neighboring gridnodes.
	     *
	     * Construct an array of old_data to reflect
	     * the xproperty value on that isopycnal at
	     * each of the neighboring gridnodes and send
	     * it to zgrid().
	     *
	     * Use the value returned at newgrid[wsq_mid]
	     * as the fitted value to be output. */
		 ...
		 if (std_depth[j] >= bottom_de[sq_out]) {
		   /* This location is deeper than the depth
		    * of the ocean here. Mask out the data at
		    * this location and this depth. */
		   xout[j] = HB_f_mask;
		 }	   
		 else if (! is_flagged((float) xwork2[wsq_mid][j], HB_f_empty) ) {
		   /* This value is not empty or dry.  Keep it! */
		   xout[j] =  (float) xwork2[wsq_mid][j];
		   isopyc[sq_out][j] = sigptr[wsq_mid][j];
		   isopyc_above = isopyc[sq_out][j];
		 }
		 else {
		   /* This value is empty. Isopycnally fit
		    * the value at this level. */		   
		   if (isopyc[sq_out][j] == 0.0) {
		     if (j != nz) {
		       /* Any depth except for the bottom. */
		   ...
		   /* Check the output for the find_isopycnal
		    * or find_bottom_isopycnal routines. */
		   if (isopyc[sq_out][j] <= 0) {
		      xout[j] = HB_f_empty;
		      continue;
		   }
		   else {
		     /* Output is OK, but need to check for
		      * density inversion */
		     if (isopyc[sq_out][j] < isopyc_above - 0.001)
		       isopyc[sq_out][j] = isopyc_above;   
		     isopyc_above = isopyc[sq_out][j];
		   } /* End of output find_isopycnal output check */ 
	       ...
	       } /* End for j looping over each depth. */
	     } /* End if do_fit. */
...
void print_usage(char *program)
{
  ...
  fprintf(stderr,"\n\nUSAGE:  %s cdf_file(s) -B<w/e/s/n> -I<x_inc>[/<y_inc>] -O<outfile> -P<properties> [-D<input_dir>] [-E<input_file_extent>] [-G] [-Q<max_iterations>] [-Ssearch_radius] [-T<tension>[b|i]] [-U<topo_xinc/yinc>] [-W<topofile>] [-V] [-h] [-:]\n\n", program);
  ...
  fprintf(stderr,"-U  xinc/yinc for topography file: Default is [0.1] \n");
  fprintf(stderr,"-V verbose.  \n");
  fprintf(stderr,"-W  full pathname of topography file. This is a binary file of global\n");
  fprintf(stderr,"     topography values stored as 2-byte integers beginning at\n");
  fprintf(stderr,"     90S, 0E. The default file is [%s]\n", BATHPATH);
  fprintf(stderr,"-V verbose.  \n");
...
int cdf_construct(struct GRID_INFO *hptr, int nprops, int *prop_indx,  char *filename, int nargs, char **arglist)
   ...
   /* Opens a cdf output file and writes an apwork2priate header,
...

{
   ...
   /* Initialize a value found flag.  This
    * integer will be augmented each time
   ...
   /* If no points were found within the search,
    * then return an empty value. For the case of
    * -S0, this routine will always return an
    * empty value. */
   if (!wflag) 
      return((double) HB_f_empty);
    
   /* Otherwise, good values were found so
