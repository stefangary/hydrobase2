...
................................................................................
               ...
               ...................................................
                    ...
                    modified by: Nathan Moore 
                                 from hb_sigma_eval.c
                                 22 aug 2001
...
____________________________________________________________________________
 ...
 hb_gamma_eval filename(_roots)   [-D<dirname>] [-E<file_extent>]
...
____________________________________________________________________________
gamma_eval determines the min, max,and mean neutral density
...
{   
   ...
   fprintf(stderr,"\n%s determines the min, max, mean, and delta values of neutral density at predefined pressure levels.\n",
program);
...
void get_hydro_data(int file)
   /*  Reads each station in a HydroBase file and computes gamma values
...
/* read each station in file ... */
 ...
 /* compute neutral density properties at each level in station ... */

      npts = hdr.nobs;
      free_and_alloc(&station.observ[(int)S0], hdr.nobs);
      compute_gamma_n(&ginfo, hdr.nobs, station.observ[(int)S0], station.observ[(int)PR],
station.observ[(int)TE],station.observ[(int)SA],hdr.lat,hdr.lon);

      free_and_alloc(&station.observ[(int)S1], hdr.nobs);
      compute_gamma_n(&ginfo, hdr.nobs, station.observ[(int)S1], station.observ[(int)PR],
station.observ[(int)TE],station.observ[(int)SA],hdr.lat,hdr.lon);

      free_and_alloc(&station.observ[(int)S2], hdr.nobs);
      compute_gamma_n(&ginfo, hdr.nobs, station.observ[(int)S2], station.observ[(int)PR],
station.observ[(int)TE],station.observ[(int)SA],hdr.lat,hdr.lon);

      free_and_alloc(&station.observ[(int)S3], hdr.nobs);
      compute_gamma_n(&ginfo, hdr.nobs, station.observ[(int)S3], station.observ[(int)PR],
station.observ[(int)TE],station.observ[(int)SA],hdr.lat,hdr.lon);

      free_and_alloc(&station.observ[(int)S4], hdr.nobs);
      compute_gamma_n(&ginfo, hdr.nobs, station.observ[(int)S4], station.observ[(int)PR],
station.observ[(int)TE],station.observ[(int)SA],hdr.lat,hdr.lon);
