...
.                 p,t,s
.   
.   USAGE: apnc_convert infile_list  [-B<badfile>] [-D<dir>] [-E<extent>] 
...
int    lverbose;
int    npropsout;
int    outfile, tfile, no_qcfile;
int    staread, staout, stabad, sta_tonly;
int    topt, qopt, sta_noqc;
...
main (int  argc, char **argv)
{
   ...
   int    i,j, error, nfiles, curfile, status;
   char   *dir, *extent;
   FILE   *fp;
...
/*  parse the command line arguments */
      ...
      if (argv[i][0] == '-') {
	    ...
	    qopt = 1;
          ...
          case 'T':                    /* open  file for temp_only output */
            ...
            fprintf(stderr,"\nTemp-only profiles will be written to: %s", &argv[i][2]);
            topt = 1;
            break;
 ...
 /*  loop for each input file */
  do {
    ...
    if (status != EOF ) {
       fprintf(stderr, "WARNING:  apnc_read() returned value other than EOF\n");
    }
...
fprintf(stderr,"\n  %d stations had only pr,te profiles \n\n", sta_tonly);
exit(0);
...
{
   fprintf(stderr,"%s converts QC'd observed profile data from the ARGO \n", program);
   fprintf(stderr,"Experiment into HydroBase2 station format. \n");
   fprintf(stderr,"\nUSAGE:  %s infile_list [-O<outfile>]");
   fprintf(stderr,"\n   [-D<dir>] [-E<extent>] ");
   fprintf(stderr,"\n  [-T<filename>] [-V<detail>] ");
   ...
   fprintf(stderr,"\n    [-T]:  file to store temp-only profiles");
...
/* Begin by reading  parameters that apply to entire file */   
 ...
 /********** End file initialization. ***********/
  ...
  /* Loop for each profile: check for quality and adjusted values. */
   ...
   if (lverbose > 0)  fprintf (stderr, "  PLATFORM: %s   %ld\n", ctem, item);
   ctem[item] = '\0';
 ...
 /* PRESSURE */
   ...
   if (staOK) {
      ...
      free(data.observ[(int)TE] );
      free(data.observ[(int)SA] );
      data.observ[(int)PR] = NULL;
      data.observ[(int)DE] = NULL;
      data.observ[(int)TE] = NULL;
    ...
    */
{
   ...
   for (i = 0; i < nlevs; ++i) {
      ...
      if ( (data.observ[(int)PR][i] >= 0.0) &&(data.observ[(int)TE][i] >= -3.0) && (data.observ[(int)SA][i] >= 0.0)) {
   ...
   if (j > 0) {
      ...
      for (i = 0; i < nlevs; ++i) {
         ...
         if ( (data.observ[(int)PR][i] >= 0.0) &&(data.observ[(int)TE][i] >= -3.0) ) {
            data.observ[(int)PR][j] = data.observ[(int)PR][i];
            data.observ[(int)TE][j] = data.observ[(int)TE][i];
            data.observ[(int)SA][j] = data.observ[(int)SA][i];
            data.observ[(int)DE][j] = hb_depth(data.observ[(int)PR][i], (double)hdr.lat);
