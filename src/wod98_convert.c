...
.   
.   USAGE: wod98_convert infile_list -B<badfile> -O<outfile> -T<instr_type> [-D<dir>] [-E<extent>]
...............................................................................
*/
#include <stdio.h>
#include <stdlib.h>
...
/* definitions and global vars from NODC's OCLDEMO program */

#define MAXTAX 25
#define MAXSEC 100
#define MAXBIO 50
#define MAXPARM 100
#define MAXSCODE 7505   /* max number of ship codes in SHIPCODE.TXT */
...
int ntsets;
int *ntloc,*ntcode,*ntval,*nterr,*nttotfig,*ntsigfig,*ntrightfig;
int *depth,*zerr,*ztotfig,*zsigfig,*zrightfig;
int *dataval,*derr,*dtotfig,*dsigfig,*drightfig;
int isize, zsize;
...
char *nodc_ship[MAXSCODE];   /* shipcode table indexed by OCL shipcode */
...
main (int  argc, char **argv)
{
   ...
   char buf[80];   /* to read in shipcode data */ 
   char *dir, *extent, *st;
   FILE *shipfile;
...
/*  parse the command line arguments */
 ...
 /* read in file shipcode.asc which is derived from the OCL supplied code
    ...
    if (shipfile == NULL) {
       fprintf(stderr,"\nYou must specify the path to OCL/NODC shipcode file. \n");
       exit(1);
    }
...
/* initialize shipcode table */
    ...
    while ((error = fscanf(shipfile,"%[^\n]", buf)) != EOF) {
          error = getc(shipfile);  /* move past LF */
...
void print_usage(char *program)
{
   ...
   fprintf(stderr,"\nUsage:  %s filelist -Ssumfile [-Ooutfile] [-D<dirname>] [-E<file_extent>] [-h]", program);
...
/*****************************************************************************/
int oclread()
  /* This code was directly imported from the CDROM supplied with WOD98 */
{
 int i,j;
...
/* READ IN # OF BYTES IN CHARACTER AND PRIMARY INVESTIGATOR FIELDS*/
...
/* READ IN EACH PARAMETER CODE AND PRIMARY INVESTIGATOR CODE
 FOR THAT PARAMETER */
    ...
    /* I fixed a bug in the OCL code here (loop used i as its counter).
       Result was segment violation in some cases */
    ...
    for ( i =0; i< *(ntloc+j); i++ ) {
     ...
     if ( (iend = extracti(1,(nttotfig+ntoff+i),(ntsigfig+ntoff+i),
                 (ntrightfig+ntoff+i), (ntval+ntoff+i), missing))
                  == -1) return iend;
     totfig=1;
     if ( (iend = extracti(0,&totfig,&sigfig,&rightfig,(nterr+ntoff+i),
              missing)) == -1) return iend;
...
int extractc(int type, int *totfig, char *cdata )
 ...
 /*  type    SET TO ZERO IF TOTFIG SUPPLIED,
             SET TO ONE IF TOTFIG READ IN 

    totfig   NUMBER OF BYTES IN CHARACTER DATA 

    cdata    ARRAY OF CHARACTER DATA */
