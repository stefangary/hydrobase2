...
................................................................................
................................................................................
* sfg added -Tq, shallowest depth sorting option.  April, 2013.
*/
#include <stdio.h>
#include <stdlib.h>
#include <ctype.h>
...
/* prototypes for locally defined functions */
   ...
   int     checkshallow(struct HYDRO_HDR *, double *, int, int); 
                          ...
                          switch (*s) {
                            ...
                            case 'd' :
                                     ...
                                     if (maxdepth < mindepth) {
                                        fprintf(stderr,"\nmindepth exceeds maxdepth in -Td\n");
                                        exit(1);
                                     }
                                     if ((s = strchr(s,'/')) == NULL)  /* check for another delimiter */
                                          break;
				       ...
				       if (*s == '\0')
				           break;
                            ...
                            case 'q' :
                                     ...
                                     error += (sscanf(s,"%d", &qmin) != 1);
                                     s = strchr(s,'/');  /* check for another delimiter*/
                                     error = (s == NULL);
                                     if (s != NULL) {
                                        ++s;  /* move past delimiter */
                                        error += (sscanf(s,"%d", &qmax) != 1);
                                     }
                                     if (error) break;
                                     if (qmax < qmin) {
                                        fprintf(stderr,"\nmindepth exceeds maxdepth in -Tq\n");
                                        exit(1);
                                     }
                                     if ((s = strchr(s,'/')) == NULL)  /* check for another delimiter */
                                          break;
...
/* Check that the user has specified enough
 * information at command line. */
   if (!topt || !nfiles) {
       print_usage(argv[0]);
...
/* Loop for each station */
	 ...
	 h.lon += 360.;
       ...
       /* Start with an OK station and check
	* for each possible flag.  If staOK
	* becomes zero during this process,
	* that station is written to the
	* outcast file.  Note that the first
	* check that fails means that no
	* successive checks are performed. */    
       staOK = 1;
	 ...
	 if (p_flag < 0)
       ...
       if (q_flag && staOK) {
	 staOK = checkshallow(&h, data.observ[(int)DE], qmin, qmax);
	 if (q_flag < 0)
	   staOK = !staOK;
       }
	 ...
	 staOK = checklevs(&h, &data, ld_flag, lp_flag, minlev, maxlev);
       ...
       /* If the station survived all checks, */
       if (staOK) {
	 /* If the user only wants header information, */
	 if (h_flag) {
	   /* Write out just the header. */
	 ...
	 /* The station did not survive the checks.
          * Augment the skipped station counter. */
	 ++nskip;

	 if (o_flag) {
	   /* The user wants to save outcast data. */
	   if (h_flag) {
	     /* The user wants just header data. */
...
void print_usage(char *program)
{
   ...
   fprintf(stderr,"\n        q = shallowest level sampled falls between min/max ex: -Tq/0/50");
   fprintf(stderr,"\n        s = ship  ex:  -TsVE/6N");
...
{
  /* Changed > to >= so that there is no overlap between boxes.
   * when using hb_extract to subdivide the data into side-by-side
...
{
  /* Checks that the bathymetry depth falls within the
   * given min and max values.  In the max check, changed
   * "<=" to "<" to prevent overlap when extracting
   * multiple depth regions. */
...
int checkdepth(struct HYDRO_HDR *hptr, double *z, int min, int max)
   ...
   /* Returns a 1 if the bottommost observation is at
    * a level between min and max (inclusive); or a 0
    * if not.  In the max check, changed "<=" to "<"
    * to prevent overlap if regions of sucessive depths
    * are computed. */
{
...
/****************************************************************************/
int checkshallow(struct HYDRO_HDR *hptr, double *z, int min, int max)

   /* Returns a 1 if the uppermost observation is
    * between min and max (inclusive); or a 0
    * if not.  In the max check, changed "<=" to "<"
    * to prevent overlap if regions of sucessive depths
    * are computed. */
{

   return ((z[0] < max) && (z[0] >= min));
