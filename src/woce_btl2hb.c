...
/*  set these default values */
    ...
    oxsub = 0;  /* do not substitute ctd ox */
      ...
      if (argv[i][0] == '-') {
            switch (argv[i][1]) {
               ...
               case 'E':                    /* get file extent */
                        extent = &argv[i][2];
                        break;
               case 'K':           /* substitute ctd Ox for bad bottle values */
                       oxsub = 1;
                        break;
...
{
   fprintf(stderr,"\nUsage:  %s infile -Ssumfile [-D<dirname>] [-E<file_extent>] [-K] [-Ooutfile] [-T] [-X] [-h]", program);
   ...
   fprintf(stderr,"\n            ex: -E.dat ");
   fprintf(stderr,"\n[-K] : Substitute CTD O2 values for bottle values that are not quality code = 2 ");
	    ...
	    data.observ[(int)TE][n] *= 1.00024;
         ...
         if (prop_avail[(int)O2]) {
	       ...
	       if ( is_o2)  /* input ox is in micromoles/kg */
                  data.observ[(int)OX][n] = ox_kg2l(data.observ[(int)O2][n], data.observ[(int)PR][n],data.observ[(int)TE][n], data.observ[(int)SA][n]);
	       ...
	       if (!is_o2) {
	        ...
	        if (!is_o2) {  /* input ox is in ml/l */
                  data.observ[(int)O2][n] = ox_l2kg(data.observ[(int)O2][n], data.observ[(int)PR][n],data.observ[(int)TE][n], data.observ[(int)SA][n]);
		}
