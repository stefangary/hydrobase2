/*  wod05_convert.c
----------------------------------------------------------------------------------
*   Reads World Ocean Database 2005 observed level hydrographic data files
*   extracts :    header info
*                 p,d,t,s,ox,n2,n3,si,p4
*   
*   USAGE: wod05_convert filelist -S<shipfile> -T<instr_type> [-B<badfile>] [-D<dirname>] [-E<extent>] [-h] [-O<outfile>]
----------------------------------------------------------------------------------
* Originally based on wod01_convert.c, part of the original
* Hydrobase2 distribution.  Additional comments and changes
* added by stefan.gary@duke.edu based on the Sep. 25, 2008
* version of wodC.c from WOD05 website.
*/
#include <stdio.h>
#include <stdlib.h>
#include <math.h>
...
/* Parse command line arguments */
    ...
    * with commas all converted to spaces. */  
    ...
    while ((error = fscanf(shipfile,"%[^\n]",buf)) == 1) {
      ...
      /* First col is index to shipcode table*/
      error = sscanf(buf,"%[^','],%[^','],%s",shipindex,shipcode,shipname);
	
      /*
      fprintf(stderr,"\n Index number is: %s",&shipindex);
      fprintf(stderr,"\n Shipcode is: %s",&shipcode);
      fprintf(stderr,"\n Ship name is: %s",&shipname);
      */
 ...
 * this routine ignores the WOD cast flags by returning 1 = OK for
 * every possible profile flag.
 ****************************************************************************/
