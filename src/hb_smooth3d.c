...
...................................................
*
*  Smooths each point in HydroBase cdf files 
...
*  default = 100 km.
*  
*  Output is a HydroBase2 cdf file.  
...
int ij2xy(struct GRID_INFO *, int, int, float *, float *);
   ...
   for (i = 0; i < nprops; ++i) {
	  ...
	  default:
	   ...
	   for ( sq = 0; sq < nwsq1; ++sq) {
        ...
        /* Visit each gridpoint in output file .  
           ...
           Apply the laplacian filter and produce output profile 
	 ...
	 /* set pointers to define small work grids */
	      ...
	      if (is_pr) {
                  ...
                  get_weights(weights, bdwork2, lengthscale, &hwork2);
	      ...
	      if ( ! is_pr )
	       ...
	       while ( j < nz && std_depth[j] < mlptr[sq_out]->depth) {
		         ...
		         curr_depth = std_depth[j];
	                  ...
	                  xsmooth[j] = filter2d(zin, weights, HB_f_empty,HB_f_mask,  xrad, yrad, hwork2.nx);
                          free((void *)zin);
		     }
		   }
	         }
	 ...
	 write_prop_count_cdf(outfile, out_count, get_prop_mne(prop_indx[i]), row, col, tbin, 0, 1,1,1,NSTDLEVS);
...
void print_usage(char *program)
{
  ...
  fprintf(stderr,"      Default is [-L%5.2lf]\n", lengthscale);
 ...
 */
{
  ...
  if   (L <= 0) {   /*lengthscale not defined */
 ...
 /* compute distance and weight at each point in array */ 
  for (row = 0; row < hptr->ny; ++row) {
     for (col = 0; col < hptr->nx; ++col) {
        ...
        dx = ABS(lon - clon)*RADperDEG * cos(RADperDEG*.5*ABS(lat+clat)) * EarthRadius;
        dy = ABS(lat - clat) * RADperDEG * EarthRadius;
        dist = sqrt(dx*dx + dy*dy);
	dist = dist / L;
	weights[sq] = exp(-PI * (dist*dist));
     }
   }
    ...
    */     
{
 ...
 /* If surface value of sigma profile being smoothed is flagged... flag the mixed layer and return */   
   ...
   mixlayptr->density = filter2d(xsurf, weights, HB_f_empty, HB_f_mask, xrad, yrad, hptr->nx);
   mixlayptr->depth =  filter2d(mldepths, weights, HB_f_empty, HB_f_mask, xrad, yrad, hptr->nx);
   mixlayptr->nobs = n;
   free(xsurf);
   free(mldepths);
   return;
